...
import { updateTurma } from "@/services/turmas/turmas.client";
import { FormEvent } from "react";
import { useState } from "react";
import { getPositiveNumber, getRequiredFormString } from "@/utils/forms-utils";

type UseUpdateTurmaFormParamas = {
    onSuccess?: () => void;
};

export function useUpdateTurmaForm({ onSuccess, }: UseUpdateTurmaFormParamas = {}) {
    ...
    async function handleUpdateTurma(event: FormEvent<HTMLFormElement>, turmaId: number) {
        ...
        try {
            ...
            const request = await updateTurma(data, turmaId);
